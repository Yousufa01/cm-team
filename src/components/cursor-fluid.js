/**
 * Simplified WebGL Fluid Simulation
...
function pointerPrototype() {
    ...
    this.down = false;
    this.moved = false;
    this.color = [Math.random() * 10, Math.random() * 10, Math.random() * 10];
}
...
const displayShaderSource = `
    ...
    #ifdef SHADING
        vec3 lc = texture2D(uTexture, vL).xyz;
        vec3 rc = texture2D(uTexture, vR).xyz;
        vec3 tc = texture2D(uTexture, vT).xyz;
        vec3 bc = texture2D(uTexture, vB).xyz;
...
function render(target) {
    ...
    if (target == null && config.TRANSPARENT)
        drawColor(target, normalizeColor({ r: 0, g: 0, b: 0, a: 1 }));
...
// Initialize and export function
